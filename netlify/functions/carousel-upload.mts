...
export default async (req: Request) => {
  ...
  const name = cleanName(req.headers.get("x-asset-name") || "asset.jpg");
  ...
  if (action === "finalize") {
    ...
    await store.set(`assets/${name}`, output.buffer, { metadata: { contentType } });
    for (let i = 0; i < total; i++) await store.delete(`uploads/${name}/${i}.part`);
    return Response.json({
      ok: true,
      url: `/.netlify/functions/carousel-file?name=${encodeURIComponent(name)}`,
      bytes: totalBytes,
      contentType,
    });
  }
...
export const config: Config = { path: "/api/carousel-upload" };
